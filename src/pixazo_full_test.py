...
# Reuse the existing What-If audio design primitives from the production pipeline.
# This test does NOT modify the production pipeline.
...
# Redirect only the imported module's audio output to this isolated test folder.
# The repository's real output/audio folder is never touched.
wf.AUDIO = AUDIO
...
def normalize_visual(raw, index):
    ...
    run(["ffmpeg", "-y", "-i", str(raw), "-vf", "scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,fps=30,format=yuv420p", "-t", str(SCENE_SECONDS), "-an", "-c:v", "libx264", "-preset", "veryfast", "-crf", "19", "-movflags", "+faststart", str(out)])
    return out
...
def captioned_clip(visual, scene, index):
    ...
    vf = "drawtext=fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf:" + f"text='{text}':fontcolor=white:fontsize=68:borderw=5:bordercolor=black@0.8:x=(w-text_w)/2:y=h*0.72"
    run(["ffmpeg", "-y", "-i", str(visual), "-vf", vf, "-t", str(SCENE_SECONDS), "-an", "-c:v", "libx264", "-preset", "veryfast", "-crf", "19", "-pix_fmt", "yuv420p", str(out)])
    return out


def build_audio():
    # Exact existing What-If voice, music and scene-SFX functions.
    scene_audio = []
